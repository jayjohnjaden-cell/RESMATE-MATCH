...
const mongoURI = process.env.MONGODB_URI || "mongodb://127.0.0.1:27017/resmatematch";
...
app.get("/profile/:id", async (req, res) => {
  try {
    ...
    const html = `<div class="card" style="max-width:700px;margin:auto;"><img class="profile-cover" src="${profile.picture ? '/uploads/' + profile.picture : 'https://images.unsplash.com/photo-1517486808906-6ca8b3f04846?auto=format&fit=crop&w=1200&q=80'}" alt="campus"/><div class="pill ${profile.mode}">${profile.mode === "date" ? "Date My Resmate" : "Friend My Resmate"}</div><h1>${escapeHtml(profile.alias)}</h1><p>Status: ${verificationStatus}</p><p>${escapeHtml(profile.about)}</p><div class="tags">${profile.tags.map(tag => `<span class="pill">${escapeHtml(tag)}</span>`).join('')}</div><p><strong>Anonymous profile ID:</strong> ${profile._id}</p><div class="top-actions"><a class="btn" href="/discover/${profile._id}">Start Discovering</a><a class="btn btn-blue" href="/matches/${profile._id}">See Matches</a><a class="btn btn-pink" href="/notifications/${profile._id}">Notifications</a><a class="btn btn-gray" href="/edit/${profile._id}">Edit Profile</a><a class="btn btn-gray" href="/logout">Logout</a></div></div>`;
    res.send(pageShell(profile.alias, html));
  } catch (err) {
...
app.get("/edit/:id", async (req, res) => {
  try {
    ...
    const html = `<div class="form" style="max-width:480px;margin:auto;"><h2>Edit Profile</h2><form method="POST" action="/edit/${profile._id}" enctype="multipart/form-data">` +
      ...
      `<label><input type="checkbox" name="blurred" ${profile.blurred ? 'checked' : ''}/> Blur picture until match</label>` +
      `<button type="submit">Update Profile</button></form></div>`;
    res.send(pageShell('Edit Profile', html));
  } catch (err) {
...
app.get("/discover/:profileId", async (req, res) => {
  try {
    ...
    res.send(pageShell("Discover Profiles", html));
  } catch (err) {
...
app.get("/matches/:profileId", async (req, res) => {
  try {
    ...
    res.send(pageShell("My Matches", html));
  } catch (err) {
...
app.get("/notifications/:profileId", async (req, res) => {
  try {
    ...
    res.send(pageShell("Notifications", html));
  } catch (err) {
